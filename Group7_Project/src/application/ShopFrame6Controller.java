...
public class ShopFrame6Controller {
	...
	public void checkBtn(ActionEvent event) throws IOException {
		...
		if (result) {
			...
			try {
				...
				String state = meal.getState();
				meal.deleteMeal(SharedData.ID, meal.getMealName());

				Meal newMeal = new Meal(SharedData.ID, name, numQuan, unit, numUnitPrice, note);

				newMeal.setSomeInfo("State", state);
